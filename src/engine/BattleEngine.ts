...
export class BattleEngine {
  ...
  public initializeBattle(player1Cards: string[], player2Cards: string[]): boolean {
    ...
    for (let i = 0; i < player1Cards.length; i++) {
      const cardData = DataLoader.getCharacterAbilities(player1Cards[i]);
      if (!cardData) {
    ...
    for (let i = 0; i < player2Cards.length; i++) {
      const cardData = DataLoader.getCharacterAbilities(player2Cards[i]);
      if (!cardData) {
  ...
  private createEntity(id: string, character: CharacterAbilities): BattleEntity {
    return {
      id,
      character,
      ...
      movementSpeed: 100, // units per second
      attackSpeed: 1.0,
      statusEffects: [],
  ...
  private executeAbility(caster: BattleEntity, ability: Ability, targetId?: string): void {
  ...
  private dealDamage(attacker: BattleEntity, target: BattleEntity, ability: Ability): void {
  ...
  private applyAbilityEffects(caster: BattleEntity, ability: Ability, target?: BattleEntity | null): void {
    ...
    if (effect.includes('burn') && target) {
      const burnMatch = effect.match(/burns?\s+for\s+(\d+)\s+damage\s+over\s+(\d+)/);
      if (burnMatch) {
  ...
  private applyStatusEffect(entity: BattleEntity, effect: StatusEffect): void {
    ...
    entity.statusEffects = entity.statusEffects.filter(e => e.name !== effect.name);
  ...
  public attemptAction(action: ActionInput): boolean {
    ...
    // Find the ability
    const ability = entity.character.abilities.find(a => a.name === action.abilityName);
    if (!ability) {
      return false;
    }
