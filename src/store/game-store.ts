...
export const gameActions = {
  ...
  selectRandomCards(player: "player1" | "player2", totalCards = 2) {
    ...
    for (let i = 0; i < totalCards; i++) {
      ...
      const soulBeastData =
        AllSoulBeast[selectedCardName as keyof typeof AllSoulBeast];

      // Randomly select a configuration for this card
      const configurations = soulBeastData.configurations;
      const randomConfigIndex = Math.floor(
        Math.random() * configurations.length
      );
      ...
      console.log(
        `Selected ${selectedCardName} with configuration: ${selectedConfiguration.name} (${selectedConfiguration.abilities.join(", ")})`
      );
  ...
  addToPlayer1(cardName: SoulBeastName, configIndex?: number) {
    if (
      ...
      const selectedConfiguration =
        configurations[
          typeof configIndex === "number" ? configIndex : randomConfigIndex
        ];
  ...
  removeFromPlayer1(cardName: SoulBeastName) {
    const index = gameStore.player1Cards.findIndex(
  ...
  removeFromPlayer2(cardName: SoulBeastName) {
    const index = gameStore.player2Cards.findIndex(
      (card) => card.cardName === cardName
    );
    if (index > -1) {
