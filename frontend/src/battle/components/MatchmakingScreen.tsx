...
export const MatchmakingScreen: React.FC<MatchmakingScreenProps> = ({ onBattleStart }) => {
  ...
  useEffect(() => {
    ...
    if (networkSnap.matchmaking.matchFound && networkSnap.matchmaking.readyTimeoutMs > 0) {
  ...
  }, [networkSnap.matchmaking.matchFound, networkSnap.matchmaking.readyTimeoutMs]);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-8 bg-gradient-to-br from-slate-900 to-slate-800 text-white">
        <div className="bg-white/10 rounded-xl p-8 backdrop-blur-sm border border-white/20 max-w-lg w-full">
          <h2 className="text-xl font-semibold mb-4">Connecting to Battle Server...</h2>
          {networkSnap.network.connectionError && (
            <p className="text-red-400">Error: {networkSnap.network.connectionError}</p>
          )}
        </div>
      </div>
    );
      ...
      <div className="flex flex-col items-center justify-center min-h-screen p-8 bg-gradient-to-br from-slate-900 to-slate-800 text-white">
        <div className="bg-white/10 rounded-xl p-8 backdrop-blur-sm border border-white/20 max-w-lg w-full">
          ...
          <div className="mt-4 text-center">
            <p>You: {networkSnap.matchmaking.isReady ? "✅ Ready" : "⏳ Waiting"}</p>
            <p>Opponent: {networkSnap.matchmaking.opponentReady ? "✅ Ready" : "⏳ Waiting"}</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-8 bg-gradient-to-br from-slate-900 to-slate-800 text-white">
        <div className="bg-white/10 rounded-xl p-8 backdrop-blur-sm border border-white/20 max-w-lg w-full text-center">
          ...
          <p className="mb-8">Searching for {selectedMode === "PVP" ? "another player" : "AI opponent"}...</p>
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-8 bg-gradient-to-br from-slate-900 to-slate-800 text-white">
      <div className="bg-white/10 rounded-xl p-8 backdrop-blur-sm border border-white/20 max-w-lg w-full">
        <h1 className="text-center mb-8 text-2xl font-bold">Battle Matchmaking</h1>
        
        {/* Selected Cards Display */}
        <div className="mb-8">
          <h3 className="text-lg font-semibold mb-3">Your Selected SoulBeasts:</h3>
          {gameSnap.player1Cards.length === 0 ? (
            <p className="text-red-400">No cards selected! Please go to card selection first.</p>
          ) : (
          ...
          <div className="flex justify-center mt-4 gap-4">
            <button 
              className={`bg-white/10 border-2 border-white/30 rounded-lg text-white px-6 py-3 cursor-pointer transition-all duration-300 hover:bg-white/20 ${
                selectedMode === "PVE" ? "bg-indigo-500/30 border-indigo-400" : ""
              }`}
            ...
            <button 
              className={`bg-white/10 border-2 border-white/30 rounded-lg text-white px-6 py-3 cursor-pointer transition-all duration-300 hover:bg-white/20 ${
                selectedMode === "PVP" ? "bg-indigo-500/30 border-indigo-400" : ""
              }`}
        ...
        <div className="mb-8 text-center">
          <p>Connection: 
            <span className={networkSnap.network.isConnected ? "text-green-400" : "text-red-400"}>
        ...
        <div className="text-center">
          <button 
            ...
            disabled={gameSnap.player1Cards.length === 0 || !networkSnap.network.isConnected}
          >
            Start Matchmaking
          </button>
        </div>
